    ...
    {:else if userProfile}
        <form on:submit|preventDefault={handleUpdateProfile} class="space-y-8 rounded-lg bg-white p-8 shadow-lg">
            ...
            <div class="pt-4 text-right">
                <button type="submit" class="rounded-md bg-indigo-600 px-6 py-2 text-white hover:bg-indigo-700">
                    Salva Modifiche
                </button>
            </div>
        </form>
        ...
        {#if userProfile.ruolo === 'admin'}
            <section class="mt-12">
                ...
                <div class="rounded-lg bg-white p-6 shadow-lg">
                    <h3 class="border-b pb-4 text-xl font-semibold text-gray-800">Prodotti da Rifornire (Quantità ≤ 10)</h3>
                            ...
                            {#each productsToRestock as product}
                                <div class="flex items-center justify-between rounded-md border p-4">
                                    <div class="flex items-center">
                                        <img src={product.immagine_url} alt={product.nome} class="h-12 w-12 rounded-md object-cover" />
                                        <div class="ml-4">
                                            <p class="font-semibold text-gray-800">{product.nome}</p>
                                            <p class="text-sm {product.quantita <= 0 ? 'text-red-600 font-bold' : 'text-orange-500'}">
                                                Quantità attuale: {product.quantita}
                                            </p>
                                        </div>
                                    </div>
                                    <button on:click={() => handleRestock(product._id)} class="rounded-lg bg-green-600 px-4 py-2 text-sm font-semibold text-white hover:bg-green-700">
                                        Fai Restock
                                    </button>
                                </div>
                            {/each}
                        {:else}
                            <p class="py-4 text-center text-gray-500">Nessun prodotto da rifornire. Ottimo lavoro!</p>
                        {/if}
                    </div>
                </div>
            </section>
